...
function renderMemory(memory) {
  ...
  segments.forEach((segment, index) => {
    ...
    node.style.background =
      segment.type === "free"
        ? "rgba(152, 182, 191, 0.15)"
        : pickMemoryColor(index);
    node.title = `${segment.label} • ${segment.size} units`;
    elements.memoryBar.appendChild(node);
  });
...
function pickMemoryColor(index) {
  const palette = [
    "linear-gradient(135deg, #57d2b7, #2bb6d4)",
    "linear-gradient(135deg, #67a9ff, #3e6cff)",
    "linear-gradient(135deg, #ffb866, #ff8f56)",
    "linear-gradient(135deg, #fa7ca6, #ff5a7a)",
  ];
...
document.querySelectorAll(".tick-buttons button").forEach((button) => {
  button.addEventListener("click", async () => {
    runAction("/api/tick", { steps: Number(button.dataset.steps) });
  });
});

document.getElementById("tick-custom").addEventListener("click", async () => {
  runAction("/api/tick", { steps: Number(elements.customTicks.value || 1) });
});
